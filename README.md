...
- Launcher update/runtime path is Lua/AutoPlay (`LC.exe`).
- Python downloader scripts were moved to `_legacy-python/` and are no longer used by `Update-Game.bat`.

- External Python bootstrap in `imports/bootstrap/main.py` has been removed from runtime and moved to `_legacy-python/bootstrap/main.py`.
- Launcher bootstrap/update should be implemented inside `LastChaos Launcher.autoplay` (Lua), not via external scripts.

- Removed deprecated `imports/` folder after migrating away from external Python bootstrap/downloader flow.
